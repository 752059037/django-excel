        ...
        try:
            with transaction.atomic():  # 开启事务 如果出错则全部不保存
                for i in xlsx_iter:  # 循环得到excel里的每行数据
                    ...
                    obj = BxEverydayContrastData_form(  # 创建form对象用于校验数据
                        {
                            'brand_id': content[0],
                    ...
                    if obj.is_valid():  # 如果数据正确,按照brand_id 有则更新 无则创建
                        models.BxEverydayContrastData.objects.update_or_create(
                            brand_id=obj.cleaned_data.get('brand_id'),
                            defaults=obj.cleaned_data
                        )
                    else:
                        error_data, error_msg = list(obj.errors.items())[0]
